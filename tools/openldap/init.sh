...
if [[ ! -f /etc/ldap/docker-configured ]]; then
    ...
    SLAPD_BASE="${SLAPD_BASE:-nodomain}"
...
EOF
    ...
    sed -i "s/'dc=example,dc=com'/'$SLAPD_BASE'/g" /etc/phpldapadmin/config.php
    sed -i "s/'cn=admin,dc=example,dc=com'/'cn=admin,$SLAPD_BASE'/g" /etc/phpldapadmin/config.php
